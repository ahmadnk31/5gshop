  ...
  if (event.type === "payment_intent.succeeded") {
    ...
    if (shipping) {
      ...
      addressId = address.id;
          ...
          console.error('❌ Failed to create order address:', err);
          return new NextResponse(`Failed to create order address: ${err instanceof Error ? err.message : JSON.stringify(err)}`, { status: 500 });
        }
    }
    ...
    // Save order
      ...
      data: {
        ...
        return new NextResponse(`Failed to create order: ${err instanceof Error ? err.message : JSON.stringify(err)}`, { status: 500 });
      }
    ...
    try {
      ...
      console.error('❌ General error in payment_intent.succeeded handler:', err);
      return new NextResponse(`General error in payment_intent.succeeded handler: ${err instanceof Error ? err.message : JSON.stringify(err)}`, { status: 500 });
    }
  }
